...
#View(loblolly_pheno)
#hist(loblolly_pheno$Derregressed_BV)
#hist(loblolly_pheno$Derregressed_BV, xlab="loblolly_pheno$Derregressed_BV", col="red", breaks=100)
...
# missing rate
missingdata <- apply(loblolly_geno, 2, function(x){sum(is.na(x))/length(x)})
head(missingdata)
#length(missingdata) #4853
#hist(missingdata, breaks=25)
...
#### Plot the results
```{r, eval=FALSE, echo=TRUE}
hist(missingdata, breaks=100, col="blue", xlab="SNP Missing rate")
...
```{r, eval=FALSE, echo=TRUE}
idx1 <- which(missingdata > 0.2) #155
length(idx1)
...
```{r}
library("ggplot2")
boxplot(y.hat, xlab = "GBLUP", ylab = "Prediction", main = "Prediction Accuracy Results")

boxplot(y.hat2, xlab = "RRBLUP", ylab = "Prediction", main = "Prediction Accuracy Results")


prediction2 <- cbind(y.hat, y.hat2)
head(prediction2)
boxplot(prediction2, xlab = c("1.GBLUP", "2.RRBLUP"), ylab = "Prediction", main = "Prediction Accuracy Results")
